...
def load_students_data():
    ...
    if os.path.exists(file_path):
        # قراءة أول 3 أعمدة (الاسم، الصف، الفرقة)
        df = pd.read_excel(file_path)
        # توحيد أسماء الأعمدة لتسهيل البرمجة
...
if students_df is not None:
    # التصفية بناءً على القيم الجديدة (٦ب، ١ع، إلخ)
    ...
    if current_students:
        ...
        st.info(f"لم يتم العثور على طلاب في {class_choice}. تأكد من بيانات ملف الإكسل.")
